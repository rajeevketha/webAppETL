...
export function FieldMappingEditor({
  ...
  function add() {
    ...
    onChange([
      ...
      {
        id: id("map"),
        sourceField: sourceFields[0]?.name || null,
    ...
    <div className="bg-card border border-line rounded-xl overflow-hidden">
      <div className="px-4 py-3 border-b border-line flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-ink-soft">Source columns on the left, Salesforce fields on the right. Transforms run before load.</p>
        </div>
        <button type="button" onClick={add} className="text-sm flex items-center gap-1 px-3 py-1.5 border border-line rounded-md">
                ...
                <tr key={mapping.id} className="border-t border-line align-top">
                  <td className="px-4 py-2">
                    <select
                      ...
                      onChange={(e) => update(index, { sourceField: e.target.value || null })}
                    >
                  ...
                  <td className="py-2">
                    <select
                      className="w-52 border border-line rounded-md px-2 py-1 bg-paper"
                      value={mapping.targetField}
                      onChange={(e) => update(index, { targetField: e.target.value })}
                    >
                      {writable.map((field) => (
                        <option key={field.name} value={field.name}>
                          {field.label} {field.required ? "*" : ""} {field.externalId ? "(ext id)" : ""}
                        </option>
                      ))}
                    </select>
                  </td>
